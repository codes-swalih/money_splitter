...
function assertClose(a: number, b: number, epsilon: number = 0.01, message: string = '') {
  const diff = Math.abs(a - b);
  if (diff > epsilon) {
    console.error(`❌ FAILED: ${message} | Expected ${b}, got ${a} (diff: ${diff})`);
    process.exit(1);
  }
  console.log(`✅ PASSED: ${message}`);
}
...
try {
  ...
  assertClose(calc.totalExpense, 2345.50, 0.01, 'Total expense should be 2345.50');
  assertClose(calc.shares['1'], 469.10, 0.01, 'Each share should be 469.10');
  
  // Verify sum of shares equals total
  const shareSum = Object.values(calc.shares).reduce((a, b) => a + b, 0);
  assertClose(shareSum, 2345.50, 0.01, 'Sum of shares should equal total');
} catch (e) {
...
try {
  ...
  assert(!calc.shares['2'] || calc.shares['2'] === 0, 'Person 2 should not have a share');
} catch (e) {
  console.error('❌ Test failed:', e);
  process.exit(1);
}
...
try {
  ...
  assertClose(ijasMoney!.totalPaid, 2345.50, 0.01, 'ijas paid 2345.50');
  assertClose(ijasMoney!.totalOwed, 469.10, 0.01, 'ijas owed 469.10');
  assertClose(ijasMoney!.netBalance, 1876.40, 0.01, 'ijas net balance should be 1876.40');
  ...
  assertClose(swalihMoney!.totalOwed, 469.10, 0.01, 'swalih owed 469.10');
  assertClose(swalihMoney!.netBalance, -469.10, 0.01, 'swalih net balance should be -469.10');
...
try {
  ...
  const ledger: ParticipantLedger[] = [
    { id: '1', name: 'swalih', totalPaid: 1000, totalOwed: 200, netBalance: 800 },
    { id: '2', name: 'ijas', totalPaid: 500, totalOwed: 1000, netBalance: -500 },
    { id: '3', name: 'ameen', totalPaid: 300, totalOwed: 600, netBalance: -300 },
  ];
  ...
  const positiveBalance = ledger.reduce((sum, l) => sum + Math.max(0, l.netBalance), 0);
  assertClose(totalToTransfer, positiveBalance, 0.01, 'Transfer total should equal positive balances');

  // Check settlement is minimal
  assert(settlements.length <= 2, 'Settlement should be minimal (2 transactions for 3 people with debts)');
} catch (e) {
  console.error('❌ Test failed:', e);
  process.exit(1);
}
...
try {
  ...
  Object.values(calc.shares).forEach((share) => {
    assert(share === 33.34 || share === 33.33, `Share should be 33.33 or 33.34, got ${share}`);
  });
} catch (e) {
  console.error('❌ Test failed:', e);
  process.exit(1);
...
try {
  ...
  // Total spent
  const totalSpent = 2345.50 + 8000;
  const totalOwed = ledger.reduce((sum, l) => sum + l.totalOwed, 0);
  assertClose(totalOwed, totalSpent, 0.01, 'Total owed should equal total spent');
