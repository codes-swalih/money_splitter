...
export function generateCSVExport(
  ...
  lines.push(
    `Period: ${new Date(trip.startDate).toLocaleDateString()} - ${new Date(
      trip.endDate
    ).toLocaleDateString()}`
  );
  lines.push("");
  ...
  // Expenses
  lines.push("EXPENSES");
  lines.push("Date,Payer,Category,Amount,Tax,Tip,Total,Split Type,Description");

  for (const expense of expenses) {
    const taxAmount =
      expense.tax ||
      (expense.taxPercent !== undefined
        ? (expense.amount * expense.taxPercent) / 100
        : 0);
    const tipAmount =
      expense.tip ||
      (expense.tipPercent !== undefined
        ? (expense.amount * expense.tipPercent) / 100
        : 0);
    const total = expense.amount + taxAmount + tipAmount;

    const payer =
      trip.participants.find((p) => p.id === expense.payerId)?.name ||
      expense.payerId;
  ...
  for (const settlement of settlements) {
    lines.push(
      `"${settlement.fromName}","${settlement.toName}",${settlement.amount}`
    );
  }
